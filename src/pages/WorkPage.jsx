          ...
          {filteredProjects.map((project, idx) => (
            <div key={project.id} className="group flex flex-col justify-between">
              <Link to={`/work/${project.slug}`} data-cursor="view" className="block">
                <div className="relative aspect-[16/10] overflow-hidden bg-[#0a0a0d] border border-white/10 group-hover:border-brand-blue/60 transition-colors duration-500 mb-6">
                  <img
                  ...
                  <div className="absolute top-4 left-4 bg-black/80 px-3 py-1 text-[10px] font-mono text-white tracking-widest border border-white/10">
                    {project.category}
                  </div>

                  <div className="absolute bottom-4 right-4 bg-white text-black px-3 py-1 font-mono text-xs tracking-widest font-semibold opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                    VIEW CASE STUDY &rarr;
                  </div>
                </div>

                <div className="flex items-baseline justify-between border-b border-white/10 pb-3 mb-3">
                  <h2 className="font-display font-bold text-2xl md:text-3xl text-white group-hover:text-brand-blue transition-colors">
                    {project.title}
                  </h2>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {project.techStack.slice(0, 3).map((tech, tIdx) => (
                    <span key={tIdx} className="text-[10px] font-mono text-neutral-400 bg-white/[0.04] px-2 py-0.5 border border-white/10">
                      {tech}
                    </span>
                  ))}
                </div>
